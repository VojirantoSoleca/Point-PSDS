...
class TwoViewPointcloudTransform:
    def __init__(self, base_transforms):
        self.base_transforms = base_transforms  # 传入一个 transforms.Compose 或列表
...
class MaskTransformer(nn.Module):
    def __init__(self, config, **kwargs):
        ...
        self.mask_type = "rand"  # 'block', 'rand', 'curvature'
    ...
    def _mask_center_curvature(self, center, curvature_scores, noaug=False, mask_type=0.5, mask_value=0):
        '''
            center: [B, G, 3]
            curvature_scores: [B, G]  # 每个中心点的曲率分数
            返回：
                mask: [B, G] bool，True表示被mask
        '''
        B, G, _ = center.shape
        if noaug or self.mask_ratio == 0:
    ...
    def compute_laplacian_min_scores(self, neighborhood, normalize=False, max_iter=50, tol=1e-6, eps=1e-6):
        """
        计算每个patch的第二小拉普拉斯特征值 (λ₂, Fiedler value)
        用于衡量patch的连通性强度或结构复杂度。

        Args:
            neighborhood: [B, G, M, 3] 邻域点云
            normalize: 是否使用归一化Laplacian
            max_iter: 幂迭代最大迭代次数
            tol: 收敛容差
            eps: 数值稳定性常数
        Returns:
            lambda2: [B, G] 每个patch的第二小特征值
        """
        ...
        dist = torch.cdist(patches_flat, patches_flat)  # [B*G, M, M]
        ...
        # --- 3. 幂迭代求第一小特征向量 v1 ---
        v1 = torch.randn(B * G, M, device=device)
        v1 = v1 / (v1.norm(dim=-1, keepdim=True) + eps)
        I = torch.eye(M, device=device).unsqueeze(0)
        for _ in range(max_iter):
            v1_next = torch.bmm((I - L), v1.unsqueeze(-1)).squeeze(-1)
            v1_next = v1_next / (v1_next.norm(dim=-1, keepdim=True) + eps)
            if torch.max(torch.abs(v1_next - v1)) < tol:
                break
            v1 = v1_next
        v1 = v1.unsqueeze(-1)

        # --- 4. 去掉v1分量 ---
        P = I - torch.bmm(v1, v1.transpose(1, 2))
        L_deflated = torch.bmm(torch.bmm(P, L), P)

        # --- 5. 幂迭代求第二小特征向量 v2 ---
        v2 = torch.randn(B * G, M, device=device)
        v2 = v2 / (v2.norm(dim=-1, keepdim=True) + eps)
        for _ in range(max_iter):
            v2_next = torch.bmm((I - L_deflated), v2.unsqueeze(-1)).squeeze(-1)
            # 正交化以确保与v1正交
            proj = torch.bmm(v1.transpose(1, 2), v2_next.unsqueeze(-1))[:, 0]
            v2_next = v2_next - proj * v1.squeeze(-1)
            v2_next = v2_next / (v2_next.norm(dim=-1, keepdim=True) + eps)
            if torch.max(torch.abs(v2_next - v2)) < tol:
                break
            v2 = v2_next
        ...
        # --- 6. 计算 λ₂ ---
        lambda2 = torch.bmm(torch.bmm(v2.transpose(1, 2), L), v2).squeeze(-1).squeeze(-1)
        return lambda2.reshape(B, G)

    def compute_laplacian_min_scores2(self, neighborhood, normalize=True, eps=1e-6):
        """
        GPU 批量矢量化计算每个 patch 的第二小拉普拉斯特征值 λ₂ (Fiedler value)

        Args:
            neighborhood: [B, G, M, 3] 邻域点云
            normalize: 是否使用归一化 Laplacian
            eps: 数值稳定性常数
        Returns:
            lambda2: [B, G] 每个 patch 的第二小特征值
        """
        B, G, M, _ = neighborhood.shape
        device = neighborhood.device
        N = B * G
        ...
        # --- 1. 构建邻接矩阵 ---
        patches_flat = neighborhood.reshape(N, M, 3)  # [N, M, 3]
        dist = torch.cdist(patches_flat, patches_flat)  # [N, M, M]

        # 邻接矩阵：1 / (distance + eps)，对角线置0
        adj = 1.0 / (dist + eps)
        diag_mask = torch.eye(M, device=device).bool().unsqueeze(0)  # [1, M, M]
        adj = adj.masked_fill(diag_mask, 0.0)

        # --- 2. 构建 Laplacian ---
        if normalize:
            D = torch.sum(adj, dim=-1)  # [N, M]
            D_inv_sqrt = torch.rsqrt(D + eps)
            D_inv_sqrt_mat = torch.diag_embed(D_inv_sqrt)  # [N, M, M]
            I = torch.eye(M, device=device).unsqueeze(0)  # [1, M, M]
            L = I - D_inv_sqrt_mat @ adj @ D_inv_sqrt_mat  # [N, M, M]
        else:
            D = torch.diag_embed(torch.sum(adj, dim=-1))  # [N, M, M]
            L = D - adj

        # --- 3. 批量求特征值 ---
        # torch.linalg.eigvalsh 支持 batch 矩阵
        eigvals = torch.linalg.eigvalsh(L)  # [N, M], 升序排列

        # 第二小特征值
        lambda2 = eigvals[:, 0]  # [N]
        return lambda2.reshape(B, G)  # [B, G]
    ...
    def compute_curvature_scores(self, neighborhood):
        ...
        """
        B, G, M, _ = neighborhood.shape
        pts_centered = neighborhood - neighborhood.mean(dim=2, keepdim=True)  # 中心化，广播 (B,G,M,3)

        # 转置方便后续批量矩阵乘法，变成 (B, G, 3, M)
        pts_centered_t = pts_centered.transpose(2, 3)  # (B, G, 3, M)

        # 计算协方差矩阵 Cov = (X * X^T) / (M-1)
        cov = torch.matmul(pts_centered_t, pts_centered) / (M - 1 + 1e-6)  # (B, G, 3, 3)

        # 计算特征值（批量）
        eigvals = torch.linalg.eigvalsh(cov)  # (B, G, 3), 已经升序排列

        eigvals = torch.clamp(eigvals, min=1e-12)

        curvature = eigvals[..., 0] / eigvals.sum(dim=-1)  # 最小特征值 / 特征值和  shape: (B,G)

        return curvature

    def forward(self, neighborhood, center, noaug=False, no_Mask=False, mask_type=0.5, mask_value=0):
        # 如果设置不做mask，直接编码全部token，跳过掩码生成和过滤
        if no_Mask:
            group_input_tokens = self.encoder(neighborhood)  # B G C
            batch_size, seq_len, C = group_input_tokens.size()
            pos = self.pos_embed(center)  # 对全部中心点位置编码
            x_vis = self.blocks(group_input_tokens, pos)
            x_vis = self.norm(x_vis)
            mask = torch.zeros(batch_size, seq_len, dtype=torch.bool, device=neighborhood.device)  # 全False，没mask
            return x_vis, mask
        ...
        if self.mask_type == 'rand':
            ...
            curvature_scores = self.compute_laplacian_min_scores2(neighborhood)
...
class MLPHead(nn.Module):
    ...
    def forward(self, x):
        # 支持输入是 tuple 或 tensor
        if isinstance(x, tuple):
            x = x[0]  # 取第一个元素

        # 如果输入是 3D (B, N, C)，保持维度
        if x.dim() == 3:  # token 级输入
            B, N, C = x.shape
            x = x.view(B * N, C)  # 展平最后一维
            x = self.mlp(x)
            if self.norm_last_layer:
                x = self.ln(x)
            x = x.view(B, N, -1)  # 恢复 (B, N, D)
        elif x.dim() == 2:  # 全局 pooled 输入
            x = self.mlp(x)
            if self.norm_last_layer:
                x = self.ln(x)
        else:
...
def teacher_confidence_stats_512(t_out_mask, name="view"):
    """
    检查 teacher 输出是否过度自信或不自信
    适用于 distill_dim=512
    t_out_mask: [B, M, 512] - 只包含学生 masked patch
    name: 用于打印区分视角
    """
    ...
    # 输出统计
    # print(f"[Teacher {name}] mean max_prob = {mean_max_prob:.3f}, mean entropy = {mean_entropy:.3f}")

    # 判断过度自信 / 不自信 (针对 512 维)
    max_entropy = np.log(512)  # ≈6.238
    if mean_max_prob > 0.95 or mean_entropy < 0.1:
        print(f"[Warning] Teacher {name} may be over-confident!")
    if mean_max_prob < 0.05 or mean_entropy > max_entropy * 0.9:  # 熵接近 log(D)
...
class Point_MAE(nn.Module):
    def __init__(self, config):
        ...
        self.lambda_distill = 1.0  # 或 0.5，超参数，可调
        self.teacher_encoder = None  # 训练外部设置
        ...
        base_transforms = transforms.Compose(
            [
                ...
                data_transforms.PointcloudRotate2(),
    ...
    # @torch.no_grad()
    # def update_temp(self, epoch, max_epoch):
    #     self.student_temp = get_cosine_temp(1.5, 1.0, epoch, max_epoch)
    #     self.teacher_temp = get_cosine_temp(1.0, 0.7, epoch, max_epoch)

    @torch.no_grad()
    def update_temp(self, epoch, max_epoch, t_out_mask_u_logits, t_out_mask_v_logits):
        """
        PointMAE蒸馏用温度自适应调整函数。
        - 根据teacher logits的平均熵动态调节teacher_temp（初始偏高，后期自动收紧）
        - 适合mask reconstruction + contrastive蒸馏联合训练
        """
        # --- 初始化温度 ---
        if not hasattr(self, 'teacher_temp') or self.teacher_temp is None:
            # 前期更平滑，逐步降温
            warmup_ratio = min(epoch / (0.1 * max_epoch), 1.0)
            self.teacher_temp = 1.5 - 0.5 * warmup_ratio  # 从1.5 -> 1.0递减

        cur_temp = float(self.teacher_temp)

        # --- 计算softmax概率 ---
        eps = 1e-8
        p_u = F.softmax(t_out_mask_u_logits / cur_temp, dim=-1).clamp(min=eps)
        p_v = F.softmax(t_out_mask_v_logits / cur_temp, dim=-1).clamp(min=eps)

        # --- 计算平均熵 ---
        entropy_u = -(p_u * p_u.log()).sum(dim=-1)  # [B, M]
        entropy_v = -(p_v * p_v.log()).sum(dim=-1)
        mean_entropy = 0.5 * float(entropy_u.mean() + entropy_v.mean())

        # --- 动态目标熵 ---
        D = t_out_mask_u_logits.shape[-1]
        max_entropy = math.log(D)
        # 前期target_entropy稍低，后期稍高（让teacher分布逐渐更smooth）
        schedule = min(epoch / (0.4 * max_epoch), 1.0)
        target_entropy = max_entropy * (0.5 + 0.2 * schedule)  # 0.5~0.7倍max_entropy

        # --- 动态调整teacher_temp ---
        alpha = 0.05  # 更新强度
        delta = alpha * (target_entropy - mean_entropy) / max_entropy
        new_temp = self.teacher_temp * (1 + delta)

        # --- 限制范围更宽一点 ---
        new_temp = max(0.6, min(new_temp, 2.0))
        self.teacher_temp = float(new_temp)

        # --- 同步student_temp ---
        # 让student比teacher稍锐化，避免梯度消失
        self.student_temp = max(0.5, min(self.teacher_temp * 0.8, 1.0))

        # --- 可选调试输出 ---
        # if epoch % 10 == 0:
        #     print(f"[TempUpdate] Epoch {epoch:03d} | mean_entropy={mean_entropy:.3f} "
        #           f"| target={target_entropy:.3f} | teacher_temp={self.teacher_temp:.3f} "
        #           f"| student_temp={self.student_temp:.3f}")
    ...
    def forward(self, pts, vis=False, epoch=0, **kwargs):
        ...
        mask_type = random.choice([0.1, 0.9])
        # mask_value = random.choice([0, 1])

        x_vis_u, mask_u = self.MAE_encoder(neighborhood_u, center_u, mask_type=mask_type, mask_value=0)  # B VIS C
        x_vis_v, mask_v = self.MAE_encoder(neighborhood_v, center_v, mask_type=1-mask_type, mask_value=0)  # B VIS C
        ...
        if self.teacher_initialized and epoch > 60:
            ...
            # # ======== Step 5: total loss ========
            # # detach + item() 不影响反向传播
            # recon_val = loss_mfm.detach().item()
            # distill_val = loss_dis.detach().item()
            #
            # # 更新滑动平均（在 register_buffer 中是 tensor，需要 .mul_() 和 .add_() 原地更新）
            # self.running_recon.mul_(self.momentum).add_((1 - self.momentum) * recon_val)
            # self.running_distill.mul_(self.momentum).add_((1 - self.momentum) * distill_val)
            #
            # # 动态计算 loss 权重（防止除以0）
            # scale = (self.running_recon + 1e-6) / (self.running_distill + 1e-6)
            # scale = scale.clamp(0, 1000)
