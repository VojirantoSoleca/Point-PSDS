...
def run_net(args, config, train_writer=None, val_writer=None):
    ...
    if args.resume:
        ...
        best_metrics = Acc_Metric(checkpoint['best_metrics'])
    else:
        if args.ckpts is not None:
            print('Loading model from {}'.format(args.ckpts))
    ...
    for epoch in range(start_epoch, config.max_epoch + 1):
        ...
        if epoch % args.val_freq == 0 and epoch != 0:
            ...
            if metrics.acc < 50 and epoch > 10:
                print("error")
                exit(1)
            ...
            if better:
                ...
                builder.save_checkpoint1(base_model, optimizer, epoch, metrics, best_metrics, 'ckpt-best', args, logger = logger)
                ...
                if metrics.acc > 92.1 or (better and metrics.acc > 91):
                    ...
                    if metrics_vote.better_than(best_metrics_vote):
                        ...
                        print_log(
                            "****************************************************************************************",
                            logger=logger)
                        builder.save_checkpoint1(base_model, optimizer, epoch, metrics, best_metrics_vote, 'ckpt-best_vote', args, logger = logger)

        builder.save_checkpoint1(base_model, optimizer, epoch, metrics, best_metrics, 'ckpt-last', args, logger = logger)
