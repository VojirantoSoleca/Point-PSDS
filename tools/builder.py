...
def save_checkpoint(base_model, optimizer, scheduler, epoch, metrics, best_metrics, prefix, args, logger=None, gradnorm=None, optimizer_w=None):
    if args.local_rank == 0:
        save_dict = {
            'base_model': base_model.module.state_dict() if args.distributed else base_model.state_dict(),
            'teacher_encoder': base_model.module.teacher_encoder.state_dict(),
        ...
        save_path = os.path.join(args.experiment_path, prefix + '.pth')
        torch.save(save_dict, save_path)
        print_log(f"Save checkpoint at {save_path}", logger=logger)

def save_checkpoint1(base_model, optimizer, epoch, metrics, best_metrics, prefix, args, logger = None):
    if args.local_rank == 0:
        torch.save({
                    'base_model' : base_model.module.state_dict() if args.distributed else base_model.state_dict(),
                    'optimizer' : optimizer.state_dict(),
