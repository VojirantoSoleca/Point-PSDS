...
class PointcloudRotate(object):
    def __init__(self, axis='y', angle_range=None):
        self.axis = axis
...
class PointcloudScaleAndTranslate2(object):
    def __init__(self, scale_low=0.8, scale_high=1.25, translate_range=0.1):
    ...
    def __call__(self, pc):
        ...
        for i in range(bsize):
            ...
            scale = torch.from_numpy(xyz1).float().to(pc.device)
            translate = torch.from_numpy(xyz2).float().to(pc.device)
